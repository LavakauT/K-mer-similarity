...
df <- read.delim(paste0(dir, filenames[1])) %>%
  select(1,4)
names(df) <- c('motif', 'median')
...
dap <- read_meme('/full/path/to/ArabidopsisDAPv1.meme')
...
for (i in 1:length(df)) {
  if(i == 1){
    ...
    rname <- row.names(df)
    df$motif <- rname
    df <- df[order(-df[cname]), ][1,] # top similar motifs
    df <- df[,c('motif', cname)]
    row.names(df) <- 1
...
write.table(output,
            paste0(dir, file_name, '_top_sim_dap.txt'),
            row.names = F,
            quote = F,
            sep = '\t')
...
pdf(paste0(dir, file_name, '_top_sim_dap.pdf'))
...
save.image(file = paste0(dir, paste(file_name, 'dap.RData', sep = '_')))
